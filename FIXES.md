...
- **Issue:** In local hot-seat mode the Lobby passes *all* seats (`localPlayerIds = data.playerIds`) to the room, but the room opens `localPlayerIds[0]`'s WebSocket **only**. The client dispatches actions with the *active* seat's id (e.g. `p2`), but the CRITICAL-1 fix rewrites `action.playerId` to the authenticated socket identity (`p1`). The engine therefore rejects every `p2+` action with `ACTION_REJECTED INVALID_ACTION`, so a hot-seat game stalls as soon as `p1` ends their turn. Verified against the running stack (repro script on `localhost:3000`: p1 roll → end turn → p2's roll over the p1 socket → `INVALID_ACTION`). Online mode (one seat per connection) is unaffected.
