...
**Current Focus:** Automated Computer AI Players (Phase 35) — COMPLETE. Step 1 infrastructure: `botSeats` seat flags on `Room` (persisted in Redis snapshots, excluded from `getAvailablePlayerId`), `RoomManager.allRooms()` iterator, `BotController` (1-second sweep over live rooms; re-validates each strategy action via `engine.isValidAction` before dispatch through the standard `Room.dispatch` pipeline), `POST /rooms` accepts `bots: string[]`, and a new `@packages/ai` workspace (`IBotStrategy`). Step 2 `MonopolyBot` (trades, laddered debt resolution pay→sell→mortgage→bankrupt, jail escape, buy/build/unmortgage/end). Step 3 `ScotlandYardBot` (Mr. X flees nearest detective via BFS hop-distance, preferring reserve tickets and double moves when they buy real separation — never through a reveal turn; detectives chase the revealed position or the `mrXLog`-deduced region, never blocking a teammate). Step 4 `CatanBot` per-phase heuristics (token/distance-aware initial placement, exact-count most-abundant discards, VP-leader robber placement, main-turn ladder city→settlement→dev-card→knight→bank-trade→end, favorable-only trade responses). Steps 2–4 each verified end-to-end via bot-vs-bot `BotGameplay.test.ts`. Step 5 Lobby "Computer Players (Bots)" selector (0..players-1, always fills tail seats so the creator's first seat stays human) wired to `POST /rooms` `bots`. Prior phases complete: 34 (Performance & Load Optimization — five batches of UI code-split/memo + server serialization dedup/coalescing + Redis rehydration + engine incremental recomputation + UI derived-data memoization; Post-Phase-34 fix: Monopoly now transitions `LOBBY → IN_PROGRESS` on its first successful action — see FIXES.md #20), 33 Turn Timer / AFK Management, 32 Containerization (incl. an automated git-push-triggered VPS deploy via `.github/workflows/deploy.yml` — see README "Production Deployment" / PROJECT_TRACKER Phase 32), 31 Redis persistence & server hardening.
