...
**Current Focus:** Performance & Load Optimization (Phase 34) — Batches 1–4 complete (UI code-splitting, Vite `manualChunks`, board memoization, `Dice3D` CSS extraction; server serialization dedup, Redis rehydration, engine incremental recomputation). Remaining: UI rendering/memoization batch (memoize derived data in `GameRoom`/`CatanRoom`/`MonopolyBoard`, batch token DOM work in `PlayerToken`, wire or delete dead hooks). Prior phases complete: 32 Containerization (incl. an automated git-push-triggered VPS deploy via `.github/workflows/deploy.yml` — see README "Production Deployment" / PROJECT_TRACKER Phase 32), 33 Turn Timer / AFK Management, 31 Redis persistence & server hardening.
