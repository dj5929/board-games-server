...
### 🟢 Step 3 (COMPLETE): `ScotlandYardBot` — Mr. X flee / detective chase strategy (`packages/ai/src/ScotlandYardBot.ts`)
- 🟢 **Mr. X (flee):** scores every legal move by the graph hop-distance to the *nearest* detective (computed wholesale via BFS `distanceToSetMap` over the shared `scotlandYardGraph`), plus a small bonus for spending reserve tickets (underground > bus > taxi) and routing toward low-connectivity nodes on reveal turns. Prefers a `DOUBLE_MOVE` when it buys ≥ `DOUBLE_MOVE_GAIN` separation over the best single move — and **never doubles through a reveal turn** (the engine requires Mr. X to reveal on turns 3/8/13/18/24, and a double would expose him).
- 🟢 **Detective (chase):** scores candidate one-hop moves by the hop-distance to the target set, where the target set is (a) Mr. X's **true position** from the engine's `getStateForPlayer` scrub when he is revealed / game over, or (b) the **deduced region** of nodes Mr. X could currently occupy — filtered by replaying his revealed `mrXLog` transport types back through the board. Never steps onto another detective's node; reserves expensive tickets on ties; `SKIP_TURN` safely when out of ticket-legal nodes.
- 🟢 **`BotController` wiring:** `server.ts` registers `botController.registerStrategy('scotland-yard', new ScotlandYardBot())`.
- 🟢 **Testing (TDD):** `packages/ai/test/ScotlandYardBot.test.ts` — 7 cases: Mr. X flees nearest detective preferring underground; double-move on real separation; no double on reveal; detective shortest-path chase of a revealed Mr. X; detective avoids a teammate's node; detective deduces the hidden region from the revealed transport log (verifying the engine scrub sets position 0); skip when ticketless. Plus `BotGameplay.test.ts` adds a bot-vs-bot Scotland Yard room (Mr. X + 2 detectives) that rotates through a full round via the controller.
- 🟢 **Verification:** Full suite green (**33 files / 352 tests**), root typecheck clean, root lint clean, `@packages/server` production build passes.

### 🟢 Step 4 (COMPLETE): `CatanBot` — per-phase heuristic strategy (`packages/ai/src/CatanBot.ts`)
- 🟢 **Phase-driven decision making** (all candidates shared the Monopoly-style re-validation pattern — every action re-checked with `engine.isValidAction`, and legality rules *mirror the engine* so candidates survive `reduce`):
  - **INITIAL_PLACEMENT:** settle the highest-scoring **legal** vertex (unoccupied + distance rule via the shared `boardGraph` adjacency), scoring each candidate by token-weighted hex resources (6/8 > 5/9 > … with a small shortage-adjustment weight) minus a desert penalty; then attach a road touching the pending settlement. The distance rule is enforced in the bot itself, not just the validator, so the controller never stalls on a placement tick.
  - **DISCARD_PHASE:** drops the most abundant resources first and always produces the **exact** required count (the engine rejects partial discards with `Must discard exactly N resources`).
  - **ROBBER_PLACEMENT:** targets the victory-point leader (ties broken by hand size) and only when they actually have a building adjacent to the chosen hex — otherwise moves the robber without stealing. Also skips re-placing onto the current robber hex.
  - **MAIN_TURN:** `ROLL_DICE` when not rolled, then the approved ladder `UPGRADE_CITY → BUILD_SETTLEMENT → BUY_DEV_CARD → PLAY_KNIGHT → TRADE_BANK → END_TURN`. Settlement builds pick the best legal vertex that is road-connected (matching the engine's `Must build connected to a road`); city upgrades pick the best owned settlement; bank trades only when short exactly one resource for a build *and* holding a 4+ surplus (safe against any port rate). The bot never proposes player trades, and handles incoming offers accept-only-if-clearly-favorable.
- 🟢 **`BotController` wiring:** `server.ts` registers `botController.registerStrategy('catan', new CatanBot())`, and `packages/ai/src/index.ts` re-exports it alongside the other strategies.
