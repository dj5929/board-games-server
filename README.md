...
- **Rematch / Play Again** — every game-over screen has a "Play Again" button that resets the game in-place (same room, seats, invite codes, bot seats & chat history preserved) via the engine's `RESTART_GAME` action + `GAME_RESTARTED` event (Phase 41).
...
- **Computer AI Players**: server-side bots that occupy seats and play automatically (Monopoly, Scotland Yard & Catan strategies live in `@packages/ai`); the Lobby's "Computer Players (Bots)" selector fills tail seats via `POST /rooms` `bots: string[]`, and bot seats are reserved so humans never join into one (Phase 35).
- **Spectator Mode**: read-only watch from the Lobby or the public room browser via `POST /rooms/:id/spectate`; spectator WebSockets receive the engine's hidden per-player projection with a live `spectatorCount`, and inbound spectator frames are never validated or dispatched (Phase 36).
- **Public Room Browser**: live Lobby directory (`RoomManager.listPublicRooms()` → `GET /rooms`, optional `?gameType=` filter) with per-room Join/Watch actions; rooms opt in via an `isPublic` flag that survives Redis rehydration like the hot-seat/owner flags (Phase 37).
- **Rematch / Play Again**: every game-over screen has a "Play Again" button dispatching the engine's `RESTART_GAME` action — the room resets to a fresh `getInitialState` (same players/seats/invite codes/bot seats & chat preserved) and the server rebroadcasts a `GAME_RESTARTED` event which resets each client's transient state; available in all three games (Phase 41).
