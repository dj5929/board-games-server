...
- **Computer AI Players**: server-side bots that occupy seats and play automatically (Monopoly, Scotland Yard & Catan strategies live in `@packages/ai`); `POST /rooms` accepts `bots: string[]`, and bot seats are reserved so humans never join into one (Phase 35).
