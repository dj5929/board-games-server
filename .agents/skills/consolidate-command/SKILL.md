...
5. **Commit & Push**: Stage the consolidated documentation changes (plus any code changes from the session) and review the staged diff with `git diff --cached --stat`. Write a concise conventional-commit message (matching the repo's style, e.g. `feat:`/`test:`/`docs:`) that summarizes the session's work, then `git commit` and `git push origin main`. Verify the push succeeded (`git push` output shows the branch advancing). Note: pushing to `main` also triggers the CI and the automated VPS deploy workflows.
